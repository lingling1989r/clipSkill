...
```bash
# 公开链接直接下载
node scripts/run.mjs --url "https://..." --instruction "把口头禅去掉"

# 带 Token 的私有链接（如飞书文件）
node scripts/run.mjs --url "https://..." --url-token <飞书access_token>
```
...
|------|------|--------|
| `--file` | 本地音频或视频文件路径 | — |
| `--url` | 从 HTTP URL 下载文件再上传（适合飞书等大文件） | — |
| `--url-token` | 下载 URL 所需的 Bearer Token（如飞书 access token） | — |
